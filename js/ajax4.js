 ...
 * 时间：2018-3-11
 * Edit: tuannvbg@gmail.com 2023-04-03
 *************************************************/
function timeConvert(n) {
	...
	function zingmp3(i) {
		return "http://api.mp3.zing.vn/api/streaming/audio/" + i + "/128"
	}
// ajax加载搜索结果
function ajaxSearch() {
    if(rem.wd === ""){
...
function ajaxUrl(music, callback)
{
    ...
    $.ajax({ 
        ...
        success: function(jsonData){
            ...
            } else if(music.source == "zingmp3") {    // 解决百度音乐防盗链
                jsonData.url = zingmp3(music.id);
            }
...
function ajaxPlayList(lid, id, callback) {
    ...
    $.ajax({
        ...
        success: function(jsonData){
            ...
            var tempList = {
                id: lid,    // 列表的网易云 id
                name: jsonData.data.info.name,   // 列表名字
                cover: jsonData.data.info.thumbnail_medium,   // 列表封面
                creatorName: jsonData.data.info.artists_names,   // 列表创建者名字
                creatorAvatar: jsonData.data.info.thumbnail,   // 列表创建者头像
                item: []
            };
            
            if(jsonData.data.info.thumbnail !== '') {
                tempList.cover = jsonData.data.info.thumbnail + "?param=200y200";
            } else {
                tempList.cover = musicList[id].cover;
            }
            
            if(typeof jsonData.data.items !== undefined || jsonData.data.items.length !== 0) {
                // 存储歌单中的音乐信息
				tempLrcMessage = "";
				var audioSource = "";
                for (var i = 0; i < jsonData.data.items.length; i++) {
					if (jsonData.data.items[i].album==undefined || jsonData.data.items[i].artist==undefined){
						if (jsonData.data.items[i].lyric==undefined || jsonData.data.items[i].lyric=="" || jsonData.data.items[i].lyric==null) {
						tempLrcMessage = "[00:00.00]Bài hát: " + jsonData.data.items[i].name + "\n[00:06.00]Ca sĩ: " + jsonData.data.items[i].artists_names + "\n[00:12.00]Album: " + jsonData.data.info.name + "\n[00:18.00]Bài hát hiện chưa có lời karaoke\n[00:24.00]Chúng tôi sẽ sớm cập nhật lời bài hát!\n[00:30.00]Chúc bạn nghe nhạc vui vẻ!";
							jsonData.data.items[i].lyric = tempLrcMessage;
						}
						if (jsonData.data.items[i].type=='' || jsonData.data.items[i].type==undefined || jsonData.data.items[i].type==null){
							audioSource = jsonData.data.items[i].source[128];
						} else {
							audioSource = zingmp3(jsonData.data.items[i].id);
						}
						tempList.item[i] =  {
                        id: jsonData.data.items[i].id,  // 音乐ID
                        name: jsonData.data.items[i].name,  // 音乐名字
                        artist: jsonData.data.items[i].artists_names, // 艺术家名字
                        album: jsonData.data.info.name,    // 专辑名字
                        source: "zingmp3",     // 音乐来源
                        url_id: jsonData.data.items[i].id,  // 链接ID
                        pic_id: null,  // 封面ID
                        lyric_id: jsonData.data.items[i].id,  // 歌词ID
                        pic: jsonData.data.items[i].thumbnail.replace("w94", "w360") + "?param=300y300",    // 专辑图片
                        //url: null   // mp3链接
						//url: jsonData.data.items[i].source[128],
						url: audioSource,
						lrc: jsonData.data.items[i].lyric
						};
					} else {
						if (jsonData.data.items[i].type=='' || jsonData.data.items[i].type==undefined || jsonData.data.items[i].type==null){
							audioSource = jsonData.data.items[i].source[128];
						...
						if (jsonData.data.items[i].artist==undefined){
							tempList.item[i] =  {
							id: jsonData.data.items[i].id,  // 音乐ID
							name: jsonData.data.items[i].name,  // 音乐名字
							artist: jsonData.data.items[i].artists_names, // 艺术家名字
							album: jsonData.data.items[i].album.name,    // 专辑名字
							source: "zingmp3",     // 音乐来源
							url_id: jsonData.data.items[i].id,  // 链接ID
							pic_id: null,  // 封面ID
							lyric_id: jsonData.data.items[i].id,  // 歌词ID
							pic: jsonData.data.items[i].thumbnail + "?param=300y300",    // 专辑图片
							//url: null   // mp3链接
							//url: jsonData.data.items[i].source[128],
							url: audioSource,
							lrc: jsonData.data.items[i].lyric
							...
							if (jsonData.data.items[i].lyric==undefined || jsonData.data.items[i].lyric=="" || jsonData.data.items[i].lyric==null) {
						tempLrcMessage = "[00:00.00]Bài hát: " + jsonData.data.items[i].name + "\n[00:06.00]Ca sĩ: " + jsonData.data.items[i].artists_names + "\n[00:12.00]Album: " + jsonData.data.items[i].album.name + "\n[00:18.00]Bài hát hiện chưa có lời karaoke\n[00:24.00]Chúng tôi sẽ sớm cập nhật lời bài hát!\n[00:30.00]Chúc bạn nghe nhạc vui vẻ!";
							jsonData.data.items[i].lyric = tempLrcMessage;
						}
							tempList.item[i] =  {
							id: jsonData.data.items[i].id,  // 音乐ID
							...
							pic_id: null,  // 封面ID
							lyric_id: jsonData.data.items[i].id,  // 歌词ID
							pic: jsonData.data.items[i].artist.thumbnail + "?param=300y300",    // 专辑图片
							//url: null   // mp3链接
							//url: jsonData.data.items[i].source[128],
							url: audioSource,
							lrc: jsonData.data.items[i].lyric
...
function ajaxLyric(music, callback) {
    lyricTip('歌词加载中...');
    tempLRCMessage = "[00:00.00]" + music.name + "\n[00:06.00]Ca sĩ: " + music.artists_names + "\n[00:12.00]Album: " + music.album + "\n[00:18.00]Bài hát hiện chưa có lời karaoke\n[00:24.00]Chúng tôi sẽ sớm cập nhật lời bài hát!\n[00:30.00]Chúc bạn nghe nhạc vui vẻ!";
    //if(!music.lyric_id) callback('');  // 没有歌词ID，直接返回
	console.debug(tempLRCMessage);
	tempURL = '';
	tempdataType = 'json';
	tempMethod = 'GET';
	tempData = "";
    if(music.lrc!=null){
		...
		tempURL = mkPlayer.api;
		tempdataType = 'jsonp';
		tempMethod = mkPlayer.method;
	}
	lyricTip('lrc: '+tempURL);
	if (tempURL.indexOf('.lrc')>= 0){
		$.ajax({
			url: tempURL,
			success: function(templrc){
				callback(templrc, music.lyric_id);   
			},
			error: function(){
				callback(tempLRCMessage, music.lyric_id);    // 回调函数
			}
		})
	} else {
		$.ajax({
...
function ajaxLyric1(music, callback) {
    lyricTip('歌词加载中...');
	tempLRCMessage = "[00:00.00]" + music.name + "\n[00:06.00]Ca sĩ: " + music.artists_names + "\n[00:12.00]Album: " + music.album + "\n[00:18.00]Bài hát hiện chưa có lời karaoke\n[00:24.00]Chúng tôi sẽ sớm cập nhật lời bài hát!\n[00:30.00]Chúc bạn nghe nhạc vui vẻ!";
	//lyricTip(tempLRCMessage);
	console.debug("ajaxLyrric1: "+ tempLRCMessage);
    tempType = '';
